...
public final class SimulaScanner extends DefaultScanner {
	...
	public Token nextToken() {
		...
		if (SimOption.internal.TRACE_SCAN) Util.TRACE("Item.nextToken, " + edcurrent());
		return (token);
	}
    ...
    private Token scanBasic() {
    	if(SimOption.internal.TRACE_SCAN) Util.TRACE("SimulaScanner.scanBasic, "+edcurrent());
    	while(true)	{
	...
	private Token scanIdentifier() {
	    ...
	    if(SimOption.internal.TRACE_SCAN) Util.TRACE("scanIdentifier: name=\""+name+"\"");
    ...
    private Token scanNumber() {
    	...
    	if(SimOption.internal.TRACE_SCAN) Util.TRACE("scanNumber, "+edcurrent());
    	...
    	if(getNext() == 'R' && (firstChar == '2' | firstChar == '4' | firstChar == '8')) {
    		radix=firstChar - '0';
    		if(SimOption.internal.TRACE_SCAN) Util.TRACE("scanNumber, radix="+radix);
    		number.setLength(0);
    	...
    	if(SimOption.internal.TRACE_SCAN) Util.TRACE("scanNumber, result='"+result+"' radix="+radix);
    ...
    private Token scanDotDigit(StringBuilder number) {
    	if(SimOption.internal.TRACE_SCAN) Util.TRACE("scanDotDigit, "+edcurrent());
    	number.append('.');
    	...
    	String result=number.toString(); number=null;
    	if(SimOption.internal.TRACE_SCAN) Util.TRACE("scanDotDigit, result='"+result);
    	pushBack(current);
    	try {
    ...
    private Token scanDigitsExp(StringBuilder number) {
    	...
    	if(SimOption.internal.TRACE_SCAN) Util.TRACE("scanDigitsExp, "+edcurrent());
    	...
    	if(SimOption.internal.TRACE_SCAN) Util.TRACE("scanDigitsExp, result='"+result);
    	pushBack(current);
    	try {
    ...
    private String scanName() {
    	...
    	if(SimOption.internal.TRACE_SCAN) Util.TRACE("scanName, "+edcurrent());
    	...
    	while ((Character.isLetter(getNext()) || Character.isDigit(current) || current == '_'))
    		name.append((char)current);
    	pushBack(current);
    	if(SimOption.internal.TRACE_SCAN) Util.TRACE("scanName, name="+name+",current="+edcurrent());
    ...
    private Token scanCharacterConstant() {
    	char result=0;
    	if(SimOption.internal.TRACE_SCAN) Util.TRACE("scanCharacterConstant, "+edcurrent());
    	...
    	if(SimOption.internal.TRACE_SCAN) Util.TRACE("END scanCharacterConstant, result='"+result+"', "+edcurrent());
    ...
    private Token scanTextConstant() {
    	if(SimOption.internal.TRACE_SCAN) Util.TRACE("scanTextConstant, "+edcurrent());
    	...
    	LOOP:while(true) {
    		...
    		while(getNext() != '"') {
    			if(current=='!') {
    				...
    				if(SimOption.internal.TRACE_SCAN) Util.TRACE("scanTextConstant(1): Result=\""+result+"\", "+edcurrent());
    		...
    		if(getNext() == '"') {
    			...
    			if(SimOption.internal.TRACE_SCAN) Util.TRACE("scanTextConstant(2): "+edcurrent());
    			if(current!='"') {
    				pushBack(current);
    				String result=accumulatedTextConstant.toString(); accumulatedTextConstant=null;
    				if(SimOption.internal.TRACE_SCAN) Util.TRACE("scanTextConstant(2): Result=\""+result+"\", "+edcurrent());
    ...
    private int scanPossibleIsoCode() {
		...
		if (SimOption.internal.TRACE_SCAN) Util.TRACE("scanPossibleIsoCode, " + edcurrent());
			...
			if (Character.isDigit(getNext())) {
				...
				if (Character.isDigit(getNext())) {
					...
					if (getNext() == '!') { // ! digit digit digit ! Found
						...
						if (SimOption.internal.TRACE_SCAN)
	...
	private Token scanComment() {
		...
		if (SimOption.internal.TRACE_SCAN) Util.TRACE("BEGIN scanComment, " + edcurrent());
		...
		if (SimOption.internal.TRACE_SCAN) Util.TRACE("END scanComment: " + edcurrent() + "  skipped=\"" + skipped + '"');
		if (SimOption.internal.TRACE_COMMENTS) Util.TRACE("COMMENT:\"" + skipped + "\" Skipped and replaced with a SPACE");
		return (newToken(KeyWord.COMMENT));
	}
	...
	private Token scanCommentToEndOfLine() {
		StringBuilder skipped = new StringBuilder();
		if (SimOption.internal.TRACE_SCAN) Util.TRACE("BEGIN scanCommentToEndOfLine, " + edcurrent());
		while ((getNext() != '\n') && current != EOF_MARK)
			skipped.append((char) current);
		skipped.append((char) current);
		if (SimOption.internal.TRACE_SCAN) Util.TRACE("END scanCommentToEndOfLine: " + edcurrent() + "  skipped=\"" + skipped + '"');
		if (SimOption.internal.TRACE_COMMENTS) Util.TRACE("COMMENT:\"" + skipped + "\" Skipped and replaced with a SPACE");
		return (newToken(KeyWord.COMMENT));
	}
	...
	private Token scanEndComment() {
		...
		StringBuilder skipped = new StringBuilder();
		if (SimOption.internal.TRACE_SCAN) Util.TRACE("scanEndComment, " + edcurrent());
		int firstLine = Global.sourceLineNumber;
		int lastLine = firstLine;
...
//			}
			if (current == ';') {
				if (SimOption.internal.TRACE_COMMENTS) Util.TRACE("ENDCOMMENT:\"" + skipped + '"');
				...
				|| Util.equals(name, "when") || Util.equals(name, "otherwise")) {
					pushBack(name);
					if (SimOption.internal.TRACE_COMMENTS) Util.TRACE("END-COMMENT:\"" + skipped + '"');
					if (firstLine < lastLine && (skipped.length() > 0))
						Util.warning("END-Comment span mutiple source lines");
		...
		if(editorMode && accum.length()>0) tokenQueue.add(newToken(KeyWord.COMMENT));
		if (SimOption.internal.TRACE_COMMENTS)

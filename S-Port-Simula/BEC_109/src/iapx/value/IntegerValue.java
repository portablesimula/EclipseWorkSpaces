...
//		Attribute var = (Attribute) tag.getMeaning();
		AttrDescr var = (AttrDescr) Display.lookup(tag);
...
//		IO.println("IntegerValue.div: " + other + " / " + this.value);
		int res = 0;
		if(other != null) {
			IntegerValue val2 = (IntegerValue) other;
			res = val2.value / this.value;
		} else res = 0;
//		IO.println("IntegerValue.div: " + other + " / " + this.value + " = " + res);
		if(res == 0) return null;
		return IntegerValue.of(this.type, res);
	}
...
//		IO.println("IntegerValue.div: " + other + " / " + this.value + " = " + res);
	...
	@Override
	public boolean compare(Scode relation, Value other) {
		int LHS = this.value;
...
//		Util.IERR("");
	...
	public String toString() {
		...
		if(type.isBasic) return type.ident + ':' + value;
		return "C-" +type.ident + ':' + value; 
	}

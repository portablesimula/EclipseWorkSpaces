...
//import sim.editor.TextPad;

public class SimGlobal {
...
//		Global.terminal = new TextPad();			
//		Global.terminal = new Terminal("Runtime Console");			
       	SimGlobal.loadSPortSetupProperties(); 
       	SimGlobal.loadSPortProperties();
		String SPORT_HOME = SimGlobal.sPortSetupProperties.getProperty("sPort.home","?");
		if (SPORT_HOME != null) {
			if(Option.verbose) System.out.println("Global.initiate: SPORT_HOME="+SPORT_HOME);
			String SPORT_VERSION = SimGlobal.sPortSetupProperties.getProperty("sPort.version","?");
    	...
    	String revision = SimGlobal.sPortSetupProperties.getProperty("sPort.revision","?");
    	String dated = SimGlobal.sPortSetupProperties.getProperty("sPort.setup.dated","?");
    	String releaseID=SimGlobal.sPortReleaseID+'R'+revision;
		SimGlobal.sPortVersion = "S-Port Editor ("+releaseID+ " built "+dated+" using "+getJavaID()+")";
	}
	
	public static SPortEditor editor;
	...
	public static void trySetOutputDir(File dir) {
		dir.mkdirs();
		if (dir.canWrite())
			SimGlobal.outputDir = dir;
		else {
			SimGlobal.outputDir = getTempFileDir("simulaEditor/bin");
		}
	}
...
//					Global.extLib = new File(ext);
		...
		if (workspaces.isEmpty()) {
			workspaces.add(SimGlobal.sampleSourceDir);
		}
	...
	public static void setCurrentWorkspace(File workspace) {
		if (!workspace.equals(SimGlobal.currentWorkspace)) {
...
//		Properties sPortProperties = new Properties();
		...
		SimGlobal.currentWorkspace = workspaces.getFirst();

...
public final class SimulaScanner extends DefaultScanner { 
    ...
    private Token scanBasic() {
    	...
    	while(true)	{
    		Token.lineNumberBeforeScanBasic = SimGlobal.sourceLineNumber;
    		...
    		switch(current) {
	            ...
	            case '\n':			/* NL (LF) */
	    	      if (editorMode) return(newToken(KeyWord.NEWLINE,SimGlobal.sourceLineNumber+1));
	            case ' ':
	            case '\b':			/* BS */
	            case '\t':			/* HT */
	           	//case '\v':		/* VT */
    ...
    private Token scanTextConstant() {
    	...
    	LOOP:while(true) {
    		int firstLine=SimGlobal.sourceLineNumber;
    		...
    		lastLine=SimGlobal.sourceLineNumber;
    		if(getNext() == '"') {

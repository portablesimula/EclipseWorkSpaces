...
import static sim.compiler.SimGlobal.sCodeFileName;
import static sim.compiler.SimGlobal.sourceFileName;
...
//			else if(item==redo || item==redo2) redoAction();
	...
	private void doOpenFileAction() {
        JFileChooser fileChooser = new JFileChooser(SimGlobal.currentWorkspace);
        if (fileChooser.showOpenDialog(SPortEditor.tabbedPane)==JFileChooser.APPROVE_OPTION) {
    		...
    		if(lowName.endsWith(".sim")) {
            	...
            	SimGlobal.setCurrentWorkspace(fileChooser.getCurrentDirectory());
...
//		Option.internal.DEBUGGING=true;
	...
	private void doStartRunning() {
		...
		if(file==null) {
			file=new File(SimGlobal.getTempFileDir("simula/tmp/"),"unnamed.sim");
...
//			Util.println("EditorMenu.doStartRunning: " + text);
			...
			File sourceFile=new File(SimGlobal.getTempFileDir("sim/tmp/"), name + ".sim");
			sourceFile.getParentFile().mkdirs();
			sourceFileName = sourceFile.toString();
			if(Option.verbose) Util.println("EditorMenues.doStartRunning: name=" + name + ", sourceFileName="+sourceFileName);
			...
			File sCodeFile=new File(SimGlobal.getTempFileDir("sim/tmp/"), name + ".scd");
			sCodeFile.getParentFile().mkdirs();
			sCodeFileName = sCodeFile.toString();
			
//			System.out.println("EditorMenues.doStartRunning: " + SimGlobal.editor);
//			SimGlobal.editor.setVisible(false);
			
//			if(Option.verbose)
				Util.println("\n\nEditorMenues.doStartRunning: CALL FEC: Output ==> sCodeFileName = "+sCodeFileName);
			int execCode = SimulaFEC.callSimulaFEC();
//			if(Option.verbose)
				Util.println("EditorMenues.doStartRunning: RETURN FROM FEC: ExitCode = "+execCode);
			if(execCode == 0) {
//				if(Option.verbose)
...
//				if(Option.verbose)
					Util.println("RETURN FROM BEC: ExitCode = "+execCode2);
			}
    ...
    private void selectExtLibDirAction() {
    	...
    	SimGlobal.storeSPortProperties();
    }	
    ...
    private void removeWorkspacesAction() {
    	...
    	for(File workspace:SimGlobal.workspaces) {
		...
		if(res==JOptionPane.OK_OPTION) {
			for(JCheckBox box:list) {
				if(box.isSelected()) SimGlobal.workspaces.remove(new File(box.getText()));
			}
	    	SimGlobal.storeSPortProperties();
		}
    }

...
public abstract class SkipifConstruction {
	...
	public static void ofScode() {
		...
		BecGlobal.ifDepth++;
		NamedStack<CTStackItem> SKIP_Stack = CTStack.copy("IF-Stack-Copy-"+BecGlobal.ifDepth);
		ProgramAddress END_LABEL = BecGlobal.PSEG.nextAddress();
		BecGlobal.PSEG.emit(new SVM_JUMPIF(relation, tos.type.size(), null), "JUMPIF["+BecGlobal.ifDepth+"] " + relation + ':');
		...
		// FIXUP:
		SVM_JUMP instr = (SVM_JUMP) BecGlobal.PSEG.instructions.get(END_LABEL.getOfst());
		instr.destination = BecGlobal.PSEG.nextAddress();
      	BecGlobal.PSEG.emit(new SVM_NOOP(), "ENDSKIP["+BecGlobal.ifDepth+"]:");			
		BecGlobal.ifDepth--;
	}
}

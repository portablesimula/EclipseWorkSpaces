...
//				Segment.lookup("DSEG_ADHOC02").dump("ProgramAddress.execute: ");
				BecGlobal.DSEG.dump("ProgramAddress.execute: FINAL DATA SEGMENT ");
				BecGlobal.CSEG.dump("ProgramAddress.execute: FINAL CONSTANT SEGMENT ");
				BecGlobal.TSEG.dump("ProgramAddress.execute: FINAL CONSTANT TEXT SEGMENT ");
...
//				System.out.println("ProgramAddress.execute: "+cur.getClass().getSimpleName());
	...
	public static void printInstr(String cur,boolean decr) {
		...
		ProgramAddress paddr = BecGlobal.PSC.copy();
		if(decr) paddr.ofst--;

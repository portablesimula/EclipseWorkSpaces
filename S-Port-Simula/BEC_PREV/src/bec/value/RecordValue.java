...
//		Util.IERR("");
	...
	public static RecordValue ofScode() {
		...
		RecordDescr rec = (RecordDescr) BecGlobal.getMeaning(recValue.tag);
		...
		while(Scode.accept(Scode.S_ATTR)) {
			...
			Attribute attr = (Attribute) BecGlobal.getMeaning(tag);
...
//		if(Scode.inputTrace > 3) printTree(0);
		RecordDescr recordDescr = (RecordDescr) BecGlobal.DISPL.get(recValue.tag.val);
...
//		return res;
	...
	private static int setValue(Vector<Value> target, int idx, Value value) {
		...
		if(value instanceof GeneralAddress gaddr) {
			...
			idx = setValue(target, idx, txt.emitChars(BecGlobal.TSEG));
...
//			System.out.println("RecordValue.ofScode: attrValue: txt.typee="+txt.type);
			addValue(txt.emitChars(BecGlobal.TSEG));
			addValue(null);

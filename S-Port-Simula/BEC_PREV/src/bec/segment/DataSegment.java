...
//			Util.IERR("DataSegment.load: FAILED");
	...
	public ObjectAddress emit(final Value value,final String cmnt) {
		...
		values.add(value);
		comment.add(cmnt);
		if(BecGlobal.PRINT_GENERATED_SVM_DATA)
...
//		System.out.println("DataSegment.emitDefaultValue: size="+size);
		...
		boolean option = BecGlobal.PRINT_GENERATED_SVM_DATA;
		int LIMIT = 30;
		int n = size * repCount;
		for(int i=0;i<n;i++) {
			if(BecGlobal.PRINT_GENERATED_SVM_DATA && i == LIMIT) {
				System.out.println("                                 ==> ... " + (n-LIMIT) + " more truncated");
				BecGlobal.PRINT_GENERATED_SVM_DATA = false;
			}
			emit(null, cmnt);
		}
		BecGlobal.PRINT_GENERATED_SVM_DATA = option;
	}
...
//		System.out.println("NEW IMPORT: " + this);
	...
	public void write(AttributeOutputStream oupt) throws IOException {
		if(BecGlobal.ATTR_OUTPUT_TRACE) System.out.println("DataSegment.Write: " + this + ", Size=" + values.size());
...
//		System.out.println("DataSegment.readObject: ident="+ident+", segmentKind="+segmentKind);
		...
		if(BecGlobal.ATTR_INPUT_TRACE) System.out.println("DataSegment.Read: " + seg);
		if(BecGlobal.ATTR_INPUT_DUMP) seg.dump("DataSegment.readObject: ");
//		Util.IERR("");
		return seg;
	}

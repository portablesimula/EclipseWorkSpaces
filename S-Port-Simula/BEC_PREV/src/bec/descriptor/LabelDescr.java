...
//		Tag tag = Tag.ofScode();
		Tag tag = Tag.ofScode();
		LabelDescr lab = (LabelDescr) BecGlobal.DISPL.get(tag.val);
	...
	public static LabelDescr ofLabelDef(Tag tag) {
		...
		if(lab == null) {
			...
			fix.setAddress(BecGlobal.PSEG.nextAddress());
		}
		lab.adr = BecGlobal.PSEG.nextAddress();
      	BecGlobal.PSEG.emit(new SVM_NOOP(), "LABEL " + tag);
	...
	public void write(AttributeOutputStream oupt) throws IOException {
		if(BecGlobal.ATTR_OUTPUT_TRACE) System.out.println("IntDescr.Write: " + this);
		oupt.writeKind(kind);
		tag.write(oupt);
		if(adr != null) {
...
//		System.out.println("AFTER NEW MEMADDR: "+lab);
		if(BecGlobal.ATTR_INPUT_TRACE) System.out.println("LabelDescr.Read: " + lab);
		return(lab);
	}

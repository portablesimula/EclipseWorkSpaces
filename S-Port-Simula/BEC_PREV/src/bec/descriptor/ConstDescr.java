...
//		Tag tag = Tag.ofScode();
		Tag tag = Tag.ofScode();
		ConstDescr cnst = (ConstDescr) BecGlobal.DISPL.get(tag.val);
...
//			System.out.println("ConstDescr.ofConstDef: FIXREP "+fixrep);
			RecordDescr rec = (RecordDescr) BecGlobal.getMeaning(cnst.type.tag);
...
//			System.out.println("DETTE ER EN 'ResolvedType' - HVA NÅ ?");
		...
		if(cnst.address instanceof FixupOADDR fix) {
			fix.setAddress(BecGlobal.CSEG.nextAddress());
//			Util.IERR("");
		}
		cnst.address = BecGlobal.CSEG.nextAddress();
		for(int i=0;i<repCount;i++) {
			...
			value.emit(BecGlobal.CSEG, comment);
		}
		
		if(DEBUG) {
			System.out.println("CONST.inConstant: " + cnst);
//			if(Global.traceMode > 3) cnst.print("   ");
			BecGlobal.CSEG.dump("ConstDescr.ofConstDef: ");
			BecGlobal.DSEG.dump("ConstDescr.ofConstDef: ");
//			Util.IERR("");
		}
...
//		if(value != null) {
	...
	public void write(AttributeOutputStream oupt) throws IOException {
		if(BecGlobal.ATTR_OUTPUT_TRACE) System.out.println("CONST.Write: " + this);
		oupt.writeKind(kind);

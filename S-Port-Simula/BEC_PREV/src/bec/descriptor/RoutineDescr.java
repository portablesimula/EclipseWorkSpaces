...
//		if(adr == null)	adr = new FixupAddress(Type.T_PADDR, this);
	...
	public static RoutineDescr ofRoutineSpec() {
		...
		RoutineDescr rut = (RoutineDescr) BecGlobal.DISPL.get(tag.val);
...
//		String id = Global.moduleID + '_' + prftag.ident();
		String modID = (BecGlobal.moduleID == null)? "" : (BecGlobal.moduleID + '_');
...
//		}
		...
		ProgramSegment prevPSEG = BecGlobal.PSEG; BecGlobal.PSEG = rut.PSEG;
...
//		System.out.println("RoutineDescr.ofRoutineDef: "+Global.DISPL.get(prftag.val));
		ProfileDescr prf = (ProfileDescr) BecGlobal.DISPL.get(prftag.val);
		if(prf == null) Util.IERR("Missing Profile " + Scode.edTag(prftag.val));

		Scode.inputInstr();
		int rela = prf.frameSize;
		if(BecGlobal.TRACE_ALLOC_FRAME) {
		...
		BecGlobal.PSEG.emit(new SVM_ENTER(prf.getSimpleName(), rut.localFrameSize), ""+rut);
...
//		Global.PSEG.emit(new SVM_RETURN(prf.ident, prf.returSlot), "");
		BecGlobal.PSEG.emit(new SVM_RETURN(prftag.ident(), prf.returSlot), "");
		CTStack.checkStackEmpty();
...
//		}
		if(BecGlobal.PRINT_GENERATED_SVM_CODE) {
			BecGlobal.PSEG.dump("END RoutineDescr.ofRoutineDef:: ");
		}
...
//		}
		...
		if(rut.DSEG != null) BecGlobal.routineSegments.add(rut.DSEG);
		if(rut.PSEG != null) BecGlobal.routineSegments.add(rut.PSEG);
...
//		Util.IERR("");
	...
	public void write(AttributeOutputStream oupt) throws IOException {
		if(BecGlobal.ATTR_OUTPUT_TRACE) System.out.println("RoutineDescr.Write: " + this);
...
//		System.out.println("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++  BEGIN RoutineDescr.Read");
		...
		if(BecGlobal.ATTR_INPUT_TRACE) System.out.println("RoutineDescr.Read: " + rut);

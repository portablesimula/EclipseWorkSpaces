...
//		System.out.println("INDEX.ofScode: adr="+adr+"                 R"+adr.xReg);
		...
		if(adr.xReg > 0) {
			if(size > 1) {
				BecGlobal.PSEG.emit(new SVM_LOADC(Type.T_INT, IntegerValue.of(Type.T_INT, size)), "INDEX.ofScode: ");
				BecGlobal.PSEG.emit(new SVM_MULT(), "INDEX.ofScode: ");
//				Util.IERR("NOT IMPL");
			}
			BecGlobal.PSEG.emit(new SVM_ADDREG(adr.xReg), "INDEX.ofScode: ");
		} else {
			...
			BecGlobal.PSEG.emit(new SVM_STORE2REG(adr.xReg), "INDEX.ofScode: ");
		}
		if(DEBUG) BecGlobal.PSEG.dump("INDEX.ofScode: ");

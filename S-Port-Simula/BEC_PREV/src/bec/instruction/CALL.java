...
//		if(Scode.inputTrace > 3) System.out.println("CallInstruction: n="+n+", Curinstr="+Scode.edInstr(Scode.curinstr));
		ProfileDescr spec = (ProfileDescr) BecGlobal.DISPL.get(profileTag);
...
//			Global.PSEG.emit(new SVM_PRECALL(frame), ""+Scode.edTag(profileTag));
			BecGlobal.PSEG.emit(new SVM_PRECALL(spec.getSimpleName(), nParSlots, exportSize, importSize), ""+Scode.edTag(profileTag));
		}
...
//		Vector<Instruction> CALL_TOS_Instructions = null;
		...
		LOOP:while(Scode.curinstr != Scode.S_CALL) {
			...
			if(Scode.curinstr == Scode.S_ASSPAR) {
				Scode.inputInstr();
				putPar(pitem,1);
		      	BecGlobal.PSEG.emit(new SVM_NOOP(), "ASSPAR ");
		      	if(DEBUG) {
...
//			    	Util.IERR("");
				...
				Scode.inputInstr();
				putPar(pitem,nRep);
		      	BecGlobal.PSEG.emit(new SVM_NOOP(), "ASSREP " + nRep);
...
//		System.out.println("CALL.ofScode: export="+spec.exportSize);
	    if(CALL_TOS) {
	    	BecGlobal.PSEG.emit(SVM_CALL.ofTOS(spec.returSlot), "");
//    		Global.PSEG.emit(new SVM_CALL_TOS(spec.ident, spec.returSlot, spec.nParSlots, spec.exportSize, spec.importSize), ""+Scode.edTag(profileTag));
	    	...
	    	if(spec.pKind > 0) {
	    		BecGlobal.PSEG.emit(new SVM_CALL_SYS(spec.pKind), "");
	    	} else {
	    		RoutineDescr rut = (RoutineDescr) BecGlobal.DISPL.get(bodyTag);
	    		if(rut == null) Util.IERR("Unknown Routine: " + Scode.edTag(bodyTag));
	    		BecGlobal.PSEG.emit(new SVM_CALL(rut.getAddress(), spec.returSlot), ""+rut);
	    	}
	    }
		BecGlobal.PSEG.emit(new SVM_NOOP(), "Return Point");
...
//			System.out.println("CallInstruction.putPar: nrep="+nrep+", repCount="+repCount+", n="+n+", parSize="+parSize);
			for(int i=0;i<n;i++) {
				BecGlobal.PSEG.emit(new SVM_LOADC(Type.T_INT, null), "putPar: ASSREP'fill: ");

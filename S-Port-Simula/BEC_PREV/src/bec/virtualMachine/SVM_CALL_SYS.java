...
public class SVM_CALL_SYS extends SVM_Instruction {
	...
	private void initia() {
		...
		if(BecGlobal.verbose) System.out.println("SVM_SYSCALL.initia: "+exchdl);
		EXIT("INITIA: ");
	}
...
//		System.exit(code);
		if(BecGlobal.DUMPS_AT_EXIT) {
//			Segment.lookup("DSEG_ADHOC02").dump("SVM_SYSCALL.terminate: ");
			BecGlobal.DSEG.dump("SVM_SYSCALL.terminate: FINAL DATA SEGMENT ");
			BecGlobal.CSEG.dump("SVM_SYSCALL.terminate: FINAL CONSTANT SEGMENT ");
			BecGlobal.TSEG.dump("SVM_SYSCALL.terminate: FINAL CONSTANT TEXT SEGMENT ");

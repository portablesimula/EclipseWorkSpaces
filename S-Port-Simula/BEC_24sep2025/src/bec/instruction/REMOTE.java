...
//         if CurInstr=S_REMOTEV then GQfetch endif;
	...
	public static void ofScode(int instr) {
		...
		if(TESTING) {
			...
			FETCH.GQfetch("REMOTE-1 " + tag + ": ");
			Attribute attr = (Attribute) tag.getMeaning();
			RemoteAddress remadr = new RemoteAddress(attr.rela);
			AddressItem adr = AddressItem.ofREF(attr.type, attr.rela, remadr);
			adr.objState = AddressItem.Calculated;
			
			CTStack.pop();
			CTStack.push(adr);
	        if(instr == Scode.S_REMOTEV) FETCH.GQfetch("REMOTE-2 " + tag + ": ");
		} else {
...
//			CTStack.pop();
//			
//			int xReg = Reg.getFreeReg();				
//			Global.PSEG.emit(new SVM_POP2REG(xReg), "REMOTE: ");
//			DataAddress remAddr = new RemoteAddress(xReg, 0);	
//			
//			AddressItem adr = AddressItem.ofREF(attr.type, attr.rela, remAddr);
//			CTStack.push(adr);
//			

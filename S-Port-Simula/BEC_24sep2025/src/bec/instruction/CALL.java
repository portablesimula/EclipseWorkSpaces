...
//		if(Scode.inputTrace > 3) IO.println("CallInstruction: n="+n+", Curinstr="+Scode.curinstr);
...
//	      n:=TTAB(parType).nbyte; i:=nrep;
		...
		if(CTStack.TOS() instanceof AddressItem) FETCH.GQfetch("putPar: ");
		CTStack.pop();
...
//			IO.println("CallInstruction.putPar: nrep="+nrep+", repCount="+repCount+", n="+n+", parSize="+parSize);
			for(int i=0;i<n;i++) {
				Global.PSEG.emit(new SVM_PUSHC(Type.T_INT, null), "putPar: ASSREP'fill: ");

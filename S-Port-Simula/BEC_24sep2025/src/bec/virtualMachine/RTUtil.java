...
//	 end;
	...
	public static int length(SegmentAddress ent) {
		...
		switch(sort.value) {
			case S_SUB, S_PRO, S_ATT, S_DET, S_RES, S_TRM, S_PRE:
				SegmentAddress pp = (SegmentAddress) variant;
	...
	public static void dumpEntity(SegmentAddress ent) {
		IntegerValue sort = (IntegerValue) ent.addOffset(1).load();
		int lng = length(ent);
...
//	       character          cha(0);
//	 end;
	public static String entID(SegmentAddress ent) {
		try {
		IntegerValue sort = (IntegerValue) ent.addOffset(1).load();
		switch(sort.value) {
			case S_SUB, S_PRO, S_ATT, S_DET, S_RES, S_TRM, S_PRE:
				SegmentAddress pp = (SegmentAddress) ent.addOffset(3).load();
				SegmentAddress xpp = (SegmentAddress) pp.addOffset(2).load();
				if(xpp == null) return "Instance " + " " + edSort(sort.value);
				SegmentAddress idt = (SegmentAddress) xpp.load();
	...
	public static String edIDT(SegmentAddress idt, int ncha) {
	...
	public static boolean printEntity(SegmentAddress ent) {
		SegmentAddress sl = (SegmentAddress) ent.load();
		IntegerValue sort = (IntegerValue) ent.addOffset(1).load();
		...
		switch(sort.value) {
			case S_SUB, S_PRO, S_ATT, S_DET, S_RES, S_TRM, S_PRE:
				SegmentAddress pp = (SegmentAddress) variant;
				SegmentAddress gcl = (SegmentAddress) ent.addOffset(4).load();
...
//		IO.println("============ Entity " + edSort(sort.value) + " ============");
	...
	public static void dumpCurins() {
		...
		SegmentAddress curins = (SegmentAddress) rt.load(0);
...
//    end;
	...
	public static void printPool(String segID) {
		...
		SegmentAddress pool = new SegmentAddress(segID, 0);
		SegmentAddress suc = (SegmentAddress) pool.load();
		SegmentAddress nxt = (SegmentAddress) pool.addOffset(1).load();
		SegmentAddress lim = (SegmentAddress) pool.addOffset(2).load();
		SegmentAddress startgc = (SegmentAddress) pool.addOffset(3).load();
		...
		SegmentAddress ent = new SegmentAddress(segID, 7);
...
//	 end;
	...
	public static void printBasicIO() {
		...
		SegmentAddress ent = new SegmentAddress("DSEG_RT", 0);
...
//		dseg.dump("BasicIO ", 30, 60);
	...
	private static void prt(DataSegment dseg, SegmentAddress ent, String ident, int ofst) {
...
//		dseg.dump("DSEG_RT ", 0, 60);
	...
	public static void move(String src, DataAddress dst, int count) {
	...
	public static void move(DataAddress src, DataAddress dst, int count) {
		for(int i=0;i<count;i++) {
			Value x = src.load(i);
			dst.store(i, x, "MOVE DEST: "); //into.incrOffset();
		}

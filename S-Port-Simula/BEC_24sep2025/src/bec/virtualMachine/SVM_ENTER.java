...
public class SVM_ENTER extends SVM_Instruction {
	...
	@Override
	public void execute() {
		CallStackFrame frame = RTStack.precallStack.pop();
		ProgramAddress curAddr = Global.PSC.copy();
		frame.curAddr = curAddr;
		frame.localSize = localSize;
		RTStack.callStack.push(frame);
		
//		RTStack.dumpRTStack("SVM_ENTER.execute: Before push LOCAL");
		for(int i=0;i<localSize;i++) {
			RTStack.push(null, "LOCAL:");
		}
//		RTStack.dumpRTStack("SVM_ENTER.execute: After push LOCAL");

		if(Global.EXEC_TRACE > 2)
			frame.dump(""+curAddr+": ENTER: " + rutIdent + ", localSize=" + localSize);

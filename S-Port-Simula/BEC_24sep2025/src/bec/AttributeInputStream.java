...
//    	return curinstr;
    ...
    public int readOpcode() throws IOException {
    	int opcode = inpt.readByte() & 0xFF;
    	if(TRACE) IO.println("AttributeInputStream.readOpcode: "+opcode+':'+SVM_Instruction.edOpcode(opcode));
    	return opcode;
	}
    
    public int readReg() throws IOException {
    	int reg = inpt.readByte() & 0xFF;
    	if(TRACE) IO.println("AttributeInputStream.readReg: "+reg+':'+Reg.edReg(reg));
    	return reg;
	}

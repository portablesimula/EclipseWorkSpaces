...
//	public RTAddress returSlot;
//	public FrameAddress returSlot;
	public DataAddress returSlot;
...
//		IO.println("ProfileDescr.inProfile: " + Scode.edTag(ptag));
		...
		if(Scode.curinstr != Scode.S_ENDPROFILE)
			Util.IERR("Missing ENDPROFILE. Got " + Scode.curinstr);
		
		// Allocate StackFrame
		int rela = 0;
		if(prf.export != null) {
			prf.export.address = new FrameAddress(rela);
			prf.exportSize = prf.export.type.size();
			rela += prf.exportSize;
		}
		for(Variable par:prf.imports) {
			par.address = new FrameAddress(rela);
			rela += par.type.size() * par.repCount;
		}
		// Allocate Return address
		prf.returSlot = new FrameAddress(rela++);
...
//		returSlot.writeBody(oupt);
		oupt.writeShort(returSlot.ofst);
	...
	public static ProfileDescr read(AttributeInputStream inpt) throws IOException {
		...
		prf.returSlot = new FrameAddress(inpt.readShort());

...
import bec.value.dataAddress.FrameAddress;
...
//		var.address = returAddr;
	...
	public static Variable ofEXIT(DataAddress returAddr) {
		Tag tag = Tag.ofScode();
		Variable var = new Variable(Kind.K_Exit, tag);
//		var.type = new Type(Scode.TAG_PADDR);
	...
	public static Variable ofRETUR(DataAddress returSlot) {
		Variable var = new Variable(Kind.K_Retur, null);
		var.type = Type.T_PADDR;
		var.address = returSlot;
		return var;
	}
	...
	public static Variable ofLocal(int rela) {
		...
		var.type = Type.ofScode();
		var.repCount = (Scode.accept(Scode.S_REP)) ? Scode.inNumber() : 1;
//		var.address = new DataAddress(null, rela);
//		var.address = DataAddress.ofRelFrameAddr(rela);
		var.address = new FrameAddress(rela);
		return var;
	}
...
//			IO.println("NEW Global Variable: " + Scode.edTag(var.tag) + " " + var.quant + " SYSTEM " + system);
			Value value = null;
			if(system.equalsIgnoreCase("CURINS")) value = null;//new DataAddress(true);
			...
			else if(system.equalsIgnoreCase("BIOREF")) value = null;//new DataAddress(true);
...
//		address.write(oupt);
	...
	public static Variable read(AttributeInputStream inpt, int kind) throws IOException {
		...
		if(present) {
			var.address = (DataAddress) Value.read(inpt);
		}

...
%-X            call PSIMOB(smb); psc:=curins.lsc endif;
 ...
 begin ref(inst) dl;   -- A temporary to the instance dynamically
       ...
       if curins.sort = Scode.S_ATT -- set tmp, this may be NEW class
...
%-X         if bio.trc then bio.obsEvt:=EVT_END; call PSIMOB(smb) endif;
       ...
       then ---  Treat a resumed and operating object.
            ...
            ---  Invariant:  curins.sort = Scode.S_RES and curins.dl = main.sl
...
%-X         if bio.trc then bio.obsEvt:=EVT_END; call PSIMOB(smb) endif;
       ...
       elsif curins.sort = Scode.S_PRE
       then ---  Treat a prefixed block instance.
            ---  Invariant:  curins.sort = Scode.S_PRE, but check it anyway.
...
%         if ins = ref(bio) then ERROR(ENO_GOTO_1) endif;
          ...
          if ins.sort = Scode.S_ATT
          then dl:=ins.dl; ins.sort:=S_TRM; ins.dl:=none;
               ins:=dl;
          elsif ins.sort = Scode.S_RES
...
%         if dl = none then  ERROR(ENO_DET_1)  endif;
       ...
       then ins.sort:=S_DET;
            ...
            ---  Invariant:  ins.sort = Scode.S_RES, but test it anyway.
       then ins.sort := Scode.S_DET;
...
%      if ins.sort <> S_DET then ERROR(ENO_ATT_2) endif;
       ...
       assert ins.sort = Scode.S_DET skip ERROR(ENO_ATT_2) endskip;
...
%-X    assert rstr_x.sort = Scode.S_SAV skip IERR_R("RESTORE") endskip;
       curins.dl:=rstr_x.dl;

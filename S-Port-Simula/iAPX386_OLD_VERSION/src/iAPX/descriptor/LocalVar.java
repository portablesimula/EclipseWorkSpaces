...
public class LocalVar extends Descriptor {
//	int tag;  // Inherited
//	int type; // Inherited
	public int rela;
...
//	end;  
	...
	public LocalVar(Tag tag, Type type, int rela) {
	...
	public static int inLocal(int rela) { // export range(0:MaxWord) res;
		...
		rela = rela + size;
		LocalVar var = new LocalVar(tag, type, rela);
		Display.add(var);
		Util.IERR("");
		return rela;
	}
...
//		if(nextag != 0) s = s + ", nextag=" + Scode.edTag(nextag);
	...
	public static LocalVar read(AttributeInputStream inpt) throws IOException {
		LocalVar var = new LocalVar();
		LocalVar.read(var, inpt);
		if(Option.ATTR_INPUT_DUMP) IO.println("Parameter.read: "+var);
//		Util.IERR("NOT IMPL");
		return var;
	}

	protected static void read(LocalVar par, AttributeInputStream inpt) throws IOException {

...
import svm.virtualMachine.sysrut.SysDeEdit;
import svm.virtualMachine.sysrut.SysDraw;
import svm.virtualMachine.sysrut.SysEdit;
import svm.virtualMachine.sysrut.SysFile;
import svm.virtualMachine.sysrut.SysInfo;
import svm.virtualMachine.sysrut.SysKnown;
import svm.virtualMachine.sysrut.SysMath;
...
//		System.out.println("SVM_CALLSYS.stringEqual: " + str1 + " equals " + str2 + " ==> " + result);
	...
	private void printo() {
		...
		SegmentAddress chradr = (SegmentAddress) RTStack.pop();
		/*int key = */RTStack.popInt();

		SegmentAddress x = chradr.addOffset(ofst);
...
//			System.out.println("SVM_CALL.movein: idx="+i+", value="+val);
	...
	private void zeroarea() {
		...
		SegmentAddress toAddr = (SegmentAddress) RTStack.popOADDR();
		SegmentAddress fromAddr = (SegmentAddress) RTStack.popOADDR();
		SegmentAddress from = fromAddr.addOffset(0);
		SegmentAddress to = toAddr.addOffset(0);
...
//		RTStack.push(DataAddress.ofSegAddr(pool, 0) , "EXPORT");
		RTStack.push(new SegmentAddress(pool.ident, 0) , "EXPORT");
...
//		Util.IERR("");
	...
	private void dattim() {
		...
		int nchr = RTStack.popInt();
		DataAddress oaddr = RTStack.popGADDRasOADDR();
